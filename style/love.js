(function (window) {
  ...
  Seed.prototype = {
    ...
    move: function (x, y) {
      this.clear(); // Xóa vị trí cũ
      this.addPosition(x, y); // Cập nhật tọa độ mới
      this.drawCirle(); // Vẽ tại tọa độ mới
    },
    canScale: function () {
  ...
  Tree = function (canvas, width, height, opt) {
    this.canvas = canvas;
    this.ctx = canvas.getContext("2d", { willReadFrequently: true }); // Thêm tham số này
    this.width = width;
    this.height = height;
    this.opt = opt || {};
  ...
  Tree.prototype = {
    ...
    snapshot: function (k, x, y, width, height) {
      ...
      if (isEmpty) {
        x = this.seed.heart.point.x - width / 2;
        y = this.seed.heart.point.y - height / 2;
    ...
    move: function (k, x, y) {
      ...
      if (dist < speed) {
        ...
        i = point.x + (dx / dist) * speed;
        j = point.y + (dy / dist) * speed;
      }
    ...
    jump: function () {
      var s = this,
        blooms = s.blooms,
        persistentBlooms = s.persistentBlooms;
